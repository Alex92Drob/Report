...
def parse_log(start_log: Path, end_log: Path) -> dict:
    ...
    try:
        for racer in read_from_file(start_log):
            start_time_dict[racer[0:3]] = datetime.strptime(
                racer.strip()[-12:], "%H:%M:%S.%f"
            )
    ...
    for racer in read_from_file(end_log):
        end_time_dict[racer[0:3]] = datetime.strptime(
            racer.strip()[-12:], "%H:%M:%S.%f"
        )
...
def print_report(paths: list, racer_name=None, order="asc"):
    ...
    :return: None
    """
    report = build_report(paths=paths, order=order)
    headers = ["№", "ID", "NAME", "TEAM", "LAP_TIME"]
    table = []
    if racer_name:
        if racer_name not in [racer["name"] for racer in report]:
        ...
        for racer in report:
            table.append(
                [racer["place"], racer["abbr"], racer["name"], racer["car"], str(racer["time"])[:-3]]
            )
...
def cli():
    """
    This function creates CLI which have a few parameters which shows list of drivers and optional order
    (default order is asc) or shows statistic about driver you need.

    """
    parser = argparse.ArgumentParser(
        description="This application returns sorted data about Monaco Racing"
    )
    parser.add_argument(
        "--files", "-f", help="enter your path to folder with data", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--driver", "-d", type=str, help="enter driver name", default=None
    )
    args = parser.parse_args()
    if args.files is None:
        print(
            "Enter path to folder with data files: abbreviations.txt, end.log, start.log "
        )
